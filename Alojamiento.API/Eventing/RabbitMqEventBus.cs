...
public sealed class RabbitMqEventBus : IEventBus
{
    ...
    public RabbitMqEventBus(RabbitMqConnection connection, Microsoft.Extensions.Options.IOptions<RabbitMqOptions> options, ILogger<RabbitMqEventBus> logger)
    {
    ...
    public Task PublishJsonAsync(string routingKey, string payload, Guid eventId, Guid correlationId, string eventType, CancellationToken ct = default)
    {
        ...
        _logger.LogInformation(
            "Evento publicado desde Alojamiento. RoutingKey={RoutingKey}, EventId={EventId}, CorrelationId={CorrelationId}, EventType={EventType}",
            routingKey,
            eventId,
            correlationId,
            eventType);
